...
# 根据 auto_restart 参数决定容器的重启策略
    ...
    docker run -d --name $docker_name --net=$docker_host_name $restart_policy -p $docker_port:8099 -v $docker_v_music:/music -v $docker_v_cache:/cache $docker_name:v$tag_name
